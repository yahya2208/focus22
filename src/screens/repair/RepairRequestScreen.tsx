...
import { openWhatsApp, WHATSAPP_PHONE } from '../../services/whatsapp-service';
...
export const RepairRequestScreen = memo(function RepairRequestScreen() {
  ...
  const handleSubmit = useCallback(async () => {
    ...
    try {
      ...
      console.error('Failed to create repair request', e);
    } finally {
      setSubmitting(false);
    }
  ...
  const handleOpenWhatsApp = useCallback(() => {
    ...
    const message = [
      ...
      '',
      `رقم الطلب: ${result.code}`,
      `الهاتف: ${result.request.brandName} ${result.request.modelName}`,
      result.request.condition ? `الحالة: ${result.request.condition}` : '',
      '',
      `المشكلة: ${result.request.issue}`,
      description ? `${description}` : '',
      '',
      `رقم العميل: ${toInternationalFormat(normalizedPhone)}`,
      '',
      'شكراً.',
    ].filter(Boolean).join('\n');
      ...
      <Screen>
        <VStack gap="lg" style={{ alignItems: 'center', textAlign: 'center', paddingTop: '2rem' }}>
          ...
          <h2 style={{ color: '#22c55e', fontSize: '1.2rem', fontWeight: 700, margin: 0 }}>
            {t('repair.requestSubmitted') || 'تم إنشاء طلب الإصلاح'}
          </h2>
          ...
          <div style={{ display: 'flex', gap: '0.75rem', flexWrap: 'wrap', justifyContent: 'center' }}>
            <button onClick={handleCopyCode} style={{
              ...
              {copied ? '✓ ' + (t('common.copied') || 'تم النسخ') : (t('common.copy') || 'نسخ الرقم')}
            </button>
          ...
          <button onClick={handleOpenWhatsApp} style={{
            ...
            <span>💬</span> {t('repair.sendToWhatsApp') || 'إرسال التفاصيل عبر واتساب'}
          </button>
          ...
          <button onClick={() => dispatch({ type: 'NAVIGATE', screen: 'repair-home' })} style={{
            ...
            {isRtl ? '← العودة للوحة الصيانة' : '← Back to Repair'}
          </button>
        </VStack>
      </Screen>
    );
    ...
    <Screen>
      <VStack gap="md" style={{ padding: '1rem 0' }}>
        <h2 style={{ color: colors.text, fontSize: '1.1rem', fontWeight: 700, margin: '0 0 0.5rem' }}>
          🔧 {t('repair.requestRepair') || 'طلب إصلاح'}
        </h2>
        ...
        <AlgerianPhoneInput
          ...
          label={(t('repair.customerPhone') || 'رقم الهاتف') + ' *'}
          placeholder={t('repair.customerPhonePlaceholder') || '05XX XX XX XX'}
        />
        <div>
          <label style={{ ...labelStyle, color: colors.textMuted + '99' }}>
            {t('repair.customerName') || 'الاسم'} ({t('repair.optional') || 'اختياري'})
          </label>
          <input
            type="text" value={customerName} onChange={e => setCustomerName(e.target.value)}
            placeholder={t('repair.customerNamePlaceholder') || 'الاسم الكامل'}
            style={inputStyle}
          />
        </div>
        ...
        {/* Phone selection */}
        <div>
          <label style={labelStyle}>{t('repair.selectPhone') || 'اختيار الهاتف'} *</label>
        ...
        {/* Issue */}
        <div>
          <label style={labelStyle}>{t('repair.selectIssue') || 'المشكلة'} *</label>
          <select
            value={selectedIssue} onChange={e => setSelectedIssue(e.target.value)}
            style={{ ...inputStyle, appearance: 'none', cursor: 'pointer' }}
          >
            <option value="">{(t('repair.selectIssue') || 'اختر المشكلة...')}</option>
            {REPAIR_ISSUES.map(i => (
              <option key={i} value={i}>{i === 'Other' ? (t('repair.other') || 'أخرى') : i}</option>
            ))}
          </select>
        </div>
        ...
        {/* Description */}
        <div>
          <label style={labelStyle}>{t('repair.description') || 'وصف المشكلة'}</label>
          <textarea
            value={description} onChange={e => setDescription(e.target.value)}
            placeholder={t('repair.descriptionPlaceholder') || 'صف المشكلة...'}
        ...
        >
          {submitting
            ? (t('repair.submitting') || 'جاري الإرسال...')
            : (t('repair.submit') || '🔧 إنشاء طلب الإصلاح')}
        </button>
