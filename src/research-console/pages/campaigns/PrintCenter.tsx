...
export function PrintCenter({ campaignName, qrImage, logoUrl }: Props) {
  ...
  const generatePNG = async () => {
    ...
    const rows = selected.rows;
    const isSingle = layout === 'single';
    const cellW = isSingle ? canvasW - padding * 2 : Math.floor((canvasW - padding * 2) / cols);
    const cellH = isSingle ? canvasH - padding * 2 : Math.floor((canvasH - padding * 2) / rows);
    const isSticker = layout === '12sticker';
  ...
  return (
    <div>
      ...
      <div style={{ display: 'flex', gap: '0.5rem', justifyContent: 'center' }}>
        <button onClick={generatePNG} style={btnPrimary} disabled={!qrImage}>{t('campaign.downloadPNG')} (300 DPI)</button>
        <button onClick={() => window.print()} style={btnSmall}>{t('campaign.print')}</button>
      </div>
    </div>
  );
}
