...
export const BACK_MATRIX: Record<ScreenName, BackMatrixRow> = {
  ...
  results: { screen: 'results', backTarget: 'home', exitAllowed: false, browserBack: 'guard', androidBack: 'guard', hasInContentBackButton: true, note: 'P0 Correction: showroom is the ONLY post-game destination — no CTAs, no back exit; brief result display then auto-advance REPLACEs to showroom (beforeBack → false blocks back)' },
  ...
  register: { screen: 'register', backTarget: 'previous', exitAllowed: false, browserBack: 'back', androidBack: 'back', hasInContentBackButton: false, note: 'reachable from login no-account — back = previous (login)' },
  ...
  'listing-details': { screen: 'listing-details', backTarget: 'showroom', exitAllowed: false, browserBack: 'back', androidBack: 'back', hasInContentBackButton: true, note: 'P8.5 car/property details; in-content Back → showroom (deep-linkable)' },
  ...
  'tic-tac-toe-intro': { screen: 'tic-tac-toe-intro', backTarget: 'home', exitAllowed: false, browserBack: 'back', androidBack: 'back', hasInContentBackButton: false, note: 'fullscreen intro; back → home' },
  'tic-tac-toe': { screen: 'tic-tac-toe', backTarget: 'tic-tac-toe-intro', exitAllowed: false, browserBack: 'back', androidBack: 'back', hasInContentBackButton: false, note: 'fullscreen game; back → intro (quit dialog handled by useBackGuard)' },
  'tic-tac-toe-results': { screen: 'tic-tac-toe-results', backTarget: 'tic-tac-toe-intro', exitAllowed: false, browserBack: 'back', androidBack: 'back', hasInContentBackButton: false, note: 'fullscreen results; back → intro' },
  'ttt-invite-landing': { screen: 'ttt-invite-landing', backTarget: 'home', exitAllowed: false, browserBack: 'back', androidBack: 'back', hasInContentBackButton: false, note: 'invite link landing; deep-linkable, back → home' },
  'ttt-multiplayer': { screen: 'ttt-multiplayer', backTarget: 'tic-tac-toe-intro', exitAllowed: false, browserBack: 'back', androidBack: 'back', hasInContentBackButton: false, note: 'friend play board; back → intro (quit dialog via useBackGuard)' },
