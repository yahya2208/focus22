...
describe('App', () => {
  ...
  it('should render the home screen by default', async () => {
    ...
    await waitFor(() => {
      ...
      const buttons = await screen.findAllByRole('button', { name: 'Phones' }, { timeout: 10000 });
      expect(buttons.length).toBeGreaterThanOrEqual(1);
  }, TEST_TIMEOUT);

  it('should render all home screen buttons', async () => {
    renderApp();
    const buttons = await screen.findAllByRole('button', { name: 'Vegetables' }, { timeout: 10000 });
    expect(buttons.length).toBeGreaterThanOrEqual(1);
  ...
  describe('P3 Stop-Write: QR/campaign attribution removed from boot', () => {
    ...
    const buttons = await screen.findAllByRole('button', { name: 'Vegetables' }, { timeout: 10000 });
    expect(buttons.length).toBeGreaterThanOrEqual(1);
    expect(await screen.findAllByRole('button', { name: 'Phones' }, { timeout: 10000 })).toHaveLength(1);
    }, 20000);
