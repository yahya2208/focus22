-- ============================================================================
-- Migration 00033 — HOTFIX v2: get_phone_intelligence text/uuid joins
--
-- WHY A SECOND MIGRATION (evidence-driven):
--   Production still threw `42883: operator does not exist: uuid = text`
--   (CONTEXT: line 37 at RETURN) AFTER 00032 was applied, even though the
--   00032 FILE contains the fixed body. Exhaustive audit of the fixed body
--   found ZERO remaining uuid=text comparisons, and users.id is UUID on the
--   live DB (per 00008 baseline note) so `u.id = auth.uid()` is safe.
--   Conclusion: Part 2 of 00032 never actually replaced the installed
--   function — the pre-00032 body is still live. (Part 1 did land: selections
--   insert fine.) SQL Editor partial/statement-split execution is the likely
--   cause.
--
-- THIS MIGRATION:
--   Rebuilds get_phone_intelligence from the authoritative 00031 body with a
--   mechanical, diff-verified change at EXACTLY 11 join sites:
--       <agg>.device_id  =  ii.id          →   <agg>.device_id::text = ii.id::text
--   Explicit TEXT↔TEXT on both sides of every device-id correlation.
--   Signatures, authorization, aggregation logic, scoring formulas, returned
--   JSON structure and grants are byte-identical otherwise.
--
-- VERIFY INSTALLATION AFTER RUNNING (see supabase/verify/ gate):
--   SELECT prosrc FROM pg_proc WHERE proname = 'get_phone_intelligence';
--   must contain 'device_id::text' exactly 11 times.
-- ============================================================================
...
BEGIN
  ...
  RETURN jsonb_build_object(
    ...
    'top_viewed', (
      ...
      LEFT JOIN view_agg va ON va.device_id::text = ii.id::text
    ...
    'search_to_phone', (
      ...
      FROM public.inventory_items ii
      JOIN sel_agg sa ON sa.device_id::text = ii.id::text
    ),
    ...
    'whatsapp_intent', (
      ...
      FROM public.inventory_items ii
      LEFT JOIN wa_agg wa ON wa.device_id::text = ii.id::text
      WHERE ii.status NOT IN ('deleted', 'archived', 'discontinued')
        AND (v_brand_lower IS NULL OR lower(ii.brand) = v_brand_lower)
    ...
    'brand_aggregation', (
      ...
      device_scores AS (
        ...
        FROM public.inventory_items ii
        LEFT JOIN view_agg va  ON va.device_id::text  = ii.id::text
        LEFT JOIN sel_agg sa  ON sa.device_id::text  = ii.id::text
        LEFT JOIN wa_agg wa   ON wa.device_id::text   = ii.id::text
        WHERE ii.status NOT IN ('deleted', 'archived', 'discontinued')
          AND (v_brand_lower IS NULL OR lower(ii.brand) = v_brand_lower)
      )
