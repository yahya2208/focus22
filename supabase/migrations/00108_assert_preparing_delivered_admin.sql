-- ============================================================================
-- GATE V1.4 — ADMIN preparing→delivered EDGE (Vegetables Pilot).
--
-- Additive ONLY, same supersede pattern as 00090→00079: this file re-issues
-- `pilot_assert_transition` VERBATIM except for ONE added admin matrix row:
--   admin: preparing → delivered
-- enabling the atomic settle path (actuals → assert → PURCHASE) for the
-- admin-owned flow Family → Order → Admin → Prepare → External Handoff.
--
-- Preserved verbatim: signature, p_accept mode, actor resolution, atomic
-- guarded UPDATE, history writes, search_path, ACL posture, and EVERY other
-- matrix row. store_operator / courier / customer rows are byte-identical:
-- preparing → delivered stays REJECTED for all of them (courier flow still
-- requires out_for_delivery; the 00090 handoff invariant is untouched).
-- No RBAC rows, no ledger/pricing/inventory logic, no other function touched.
-- Order: after 00090 (matrix baseline). NOT applied to any DB by this file.
-- ============================================================================
...
BEGIN
  ...
  -- ===================================================================
  -- STATUS MODE: single canonical matrix. V1.4 adds exactly one row:
  -- admin preparing → delivered (atomic settle path for the admin-owned
  -- Vegetables flow). All other rows identical to 00090.
  -- ===================================================================
  v_allowed := (
    (v_role = 'admin' AND (
      ...
      OR (v_cur = 'preparing'        AND p_new_status = 'delivered')
...
-- Internal-only ACL (identical posture to 00079:688-690 and 00090): invoked
-- exclusively by the SECURITY DEFINER wrappers running as postgres.
