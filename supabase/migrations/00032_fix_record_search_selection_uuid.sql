-- ============================================================================
-- Migration 00032 — HOTFIX: text/uuid comparisons crashed analytics RPCs
--
-- Runtime evidence (production, 2026-08-22):
--   record_search_selection(12, '<real inventory uuid>', 'showroom')
--   → ERROR 42883: operator does not exist: uuid = text
--
-- Root cause: inventory_items.id is UUID while phone_search_selections /
-- phone_view_events / campaign_intents all store device_id as TEXT.
-- PostgreSQL has no implicit uuid=text operator.
--
--   Part 1: record_search_selection aborted in its device-existence check →
--           phone_search_selections could NEVER receive a row.
--   Part 2: get_phone_intelligence contained 11 joins of the form
--           'ON <agg>.device_id = ii.id' → the statement was unplanable →
--           the whole summary crashed on the first authorized staff call
--           (invisible until then: non-staff calls return UNAUTHORIZED
--           before the statement is prepared).
--
-- Fix: compare on the text side ('ii.id::text' at joins,
-- 'id::text = p_device_id' in the existence check). No signature changes,
-- aggregation logic byte-identical otherwise.
-- ============================================================================
...
BEGIN
  ...
  -- 2. Validate device exists (if provided) — uuid compared as TEXT
  IF p_device_id IS NOT NULL AND p_device_id != '' THEN
    IF NOT EXISTS (
      SELECT 1 FROM inventory_items WHERE id::text = p_device_id
    ) THEN
...
COMMENT ON FUNCTION public.record_search_selection(bigint, text, text) IS
  'Link a phone selection to its originating search event. Deduped per search_event+device. Hotfix 00032: uuid=text comparison crash (42883).';
...
-- ============================================================================
-- PART 2 — HOTFIX: get_phone_intelligence crashed identically (text = uuid)
-- ============================================================================
-- phone_view_events.device_id, campaign_intents.device_id and
-- phone_search_selections.device_id are all TEXT while inventory_items.id is
-- UUID. All 11 aggregate joins ('ON <agg>.device_id = ii.id') made the whole
-- statement unplanable: 42883 on the FIRST authorized staff call. The crash
-- was invisible because non-staff calls return UNAUTHORIZED before this
-- statement is ever prepared/planned.
-- Fix: cast inventory side to text at each join. Aggregation logic unchanged.
-- ============================================================================
...
BEGIN
  ...
  RETURN jsonb_build_object(
    ...
    'top_viewed', (
      ...
      LEFT JOIN view_agg va ON va.device_id = ii.id::text
    ...
    'search_to_phone', (
      ...
      FROM public.inventory_items ii
      JOIN sel_agg sa ON sa.device_id = ii.id::text
    ),
    ...
    'whatsapp_intent', (
      ...
      FROM public.inventory_items ii
      LEFT JOIN wa_agg wa ON wa.device_id = ii.id::text
      WHERE ii.status NOT IN ('deleted', 'archived', 'discontinued')
        AND (v_brand_lower IS NULL OR lower(ii.brand) = v_brand_lower)
    ...
    'brand_aggregation', (
      ...
      device_scores AS (
        ...
        FROM public.inventory_items ii
        LEFT JOIN view_agg va  ON va.device_id  = ii.id::text
        LEFT JOIN sel_agg sa  ON sa.device_id  = ii.id::text
        LEFT JOIN wa_agg wa   ON wa.device_id   = ii.id::text
        WHERE ii.status NOT IN ('deleted', 'archived', 'discontinued')
          AND (v_brand_lower IS NULL OR lower(ii.brand) = v_brand_lower)
      )
