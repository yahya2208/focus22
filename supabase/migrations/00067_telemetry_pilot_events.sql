-- ============================================================================
-- 00067 — FOCUS TELEMETRY PILOT EVENTS (additive, Neighborhood Pilot Phase 8)
--
-- The Neighborhood Pilot adds genuinely-NEW event names for the commerce
-- journey (neighborhood/store view, checkout, order lifecycle) plus two new
-- telemetry domains ('neighborhood', 'order'). The server allowlist in 00057
-- hard-rejects unknown event names/properties, so the DB REQUIRED an additive
-- change — exactly the pattern established by 00061 (Phase 8 auth events).
--
-- This migration is STRICTLY ADDITIVE:
--   * re-creates record_telemetry_event (full 00061 body preserved) with 9
--     additional event->domain branches + 9 event->property-allowlist branches;
--   * extends the closed domain dictionary consumed by get_telemetry_analytics
--     (00058/00061) to include 'neighborhood' and 'order';
--   * NO DROP, NO table-schema change, NO index change, NO RBAC change;
--   * does NOT modify any existing event name/property contract.
--
-- Rollback: CREATE OR REPLACE the functions back from 00057/00061 (no data
...
BEGIN
  ...
  FOR v_i IN 0 .. v_n - 1 LOOP
    ...
    CASE v_name
      ...
      WHEN 'neighborhood_view' THEN v_allowed := ARRAY[]::text[];                  -- NEW (Pilot)
      WHEN 'store_view' THEN v_allowed := ARRAY[]::text[];                  -- NEW (Pilot)
      WHEN 'family_view' THEN v_allowed := ARRAY[]::text[];                  -- NEW (Pilot)
      WHEN 'checkout_start' THEN v_allowed := ARRAY['items_count', 'with_delivery']::text[];                  -- NEW (Pilot)
      WHEN 'checkout_submit' THEN v_allowed := ARRAY['items_count']::text[];                  -- NEW (Pilot)
      WHEN 'order_created' THEN v_allowed := ARRAY['channel']::text[];                  -- NEW (Pilot)
      WHEN 'order_failed' THEN v_allowed := ARRAY['error_code']::text[];                  -- NEW (Pilot)
      WHEN 'order_status_changed' THEN v_allowed := ARRAY['status']::text[];                  -- NEW (Pilot)
      WHEN 'order_completed' THEN v_allowed := ARRAY[]::text[];                  -- NEW (Pilot)
      ELSE v_allowed := '{}'::text[];
END CASE;
...
BEGIN
  ...
  --    'auth' + Phase 8 events are NEW (00061); 'neighborhood'/'order' + the
  --    Pilot events are NEW (00067). All existing values unchanged.
  v_dom_ok := (v_domain IS NULL OR v_domain IN (
    'app','navigation','category','product','listing','cart','request','ad','game','ttt','auth','system','neighborhood','order'));
  v_ev_ok  := (v_event IS NULL OR v_event IN (
    ...
    'neighborhood_view','store_view','family_view','checkout_start','checkout_submit',
    'order_created','order_failed','order_status_changed','order_completed',
...
-- Post-apply verification — the pilot event branches must be live.
DO $$
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_proc WHERE proname = 'record_telemetry_event'
  ) THEN
    RAISE EXCEPTION '00067: record_telemetry_event missing';
  END IF;
END;
$$;
