-- ============================================================================
-- FOCUS — ADS · PER-SLIDE DESTINATIONS (PHASE 4A — ADDITIVE) — APPLY
--
...
--   The migration-format copy is supabase/migrations/00024_ads_image_destinations.sql;
--   keep the two in sync.
--
-- Type: Additive (ADD COLUMN + ADD CONSTRAINT NOT VALID + ONE NEW RPC).
...
--
-- PURPOSE
--   Give EACH ad_images row its OWN destination so every carousel slide can
--   resolve its own external / whatsapp / internal target independently of the
--   ad-level destination:
--     * ad_images.destination_type  — per-slide discriminator.
--       NULL = INHERIT the ad-level destination (the default for ALL existing
--       rows — zero re-save, zero data migration).
--       Non-NULL ∈ {external, whatsapp, internal} = this slide overrides.
--       'phone' is NOT allowed per slide: phone slides stay expressed
--       EXCLUSIVELY by ad_images.device_id (00021) — one channel per concern.
--     * ad_images.destination      — per-slide JSONB payload (mirrors the
--       ad-level `ads.destination` shape for the same type). NULL = INHERIT.
--   The DB enforces FORMAT ONLY (allowed destination_type values, NOT VALID).
--   It does NOT validate payload shape/values — the destination adapters are
--   the source of truth at render time, exactly like 00022/00023.
--
-- FALLBACK CONTRACT (Slide → Ad)
--   destination_type IS NULL AND destination IS NULL  → the slide inherits the
--   ad-level destination (resolveDestination(ad)). Non-NULL destination_type →
--   the slide builds its own adapter from its own payload. The migration adds
--   no triggers and no backfill: existing rows keep NULL/NULL → inherit.
--
-- NON-DESTRUCTIVE / SAFETY
--   * 00020/00021/00022/00023, RLS, Storage policies are NOT touched.
--   * Existing ad_images rows keep working: both new columns default to NULL.
--   * Constraint added NOT VALID (existing rows not scanned), matching the
--     ads-slide-devices two-phase pattern (no VALIDATE this cycle).
--   * ONE new RPC with a NEW NAME: ad_replace_images_destinations. It is the
--     SUPERSET of ad_replace_images_devices (same admin gate, path-prefix rule,
--     storage-object existence check, at-most-one-cover, all-or-nothing) plus
--     device_ids AND per-slide destination types/payloads — so the client can
--     write the full per-slide contract in ONE call.
--   * ad_replace_images_devices / ad_add_image_devices / ad_replace_images
--     remain UNCHANGED and executable (backward compatibility — old callers
--     keep working verbatim).
--   * No backfill, no re-run of any previous backfill.
--
-- Client side (PHASE 4B+, NOT in this file): ads-service will pass
-- destination_types[] + destinations[] to ad_replace_images_destinations when
-- any slide carries a destination; otherwise it keeps the existing RPCs.
-- ============================================================================

-- ----------------------------------------------------------------------------
-- 1) ad_images.destination_type — per-slide destination discriminator.
--    NULL (the default for every existing row) = INHERIT the ad destination.
--    Non-NULL ∈ {external, whatsapp, internal} = slide-level override.
--    'phone' is deliberately EXCLUDED: phone slides are expressed solely via
--    ad_images.device_id (00021) — never duplicated in a JSONB discriminator.
-- ----------------------------------------------------------------------------
