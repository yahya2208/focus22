-- ============================================================================
-- FOCUS — M2 · SHOWROOM ALLOWLIST FIX — LIVE APPLY (04)
--
-- THIS IS THE FILE THE OWNER RUNS ON THE LIVE DATABASE.
-- SQL is identical to 01-apply.sql; only the header differs.
--
-- Type: Additive fix for the M2 campaign-intent counters. Aligns the
--   server-side `ad_placement` allowlist with the real placements: the
--   frontend already records showroom view/click events (ShowroomScreen →
--   AdContactBanner → intent-tracking), but the base migration omitted
--   'showroom' so the RPC rejected every such event.
--
...
--   It does NOT create tables, does NOT rebuild the M2 system, does NOT touch
--   QR/Game/Results/WhatsApp, does NOT touch analytics_events, device
--   identity, campaigns or any grant model.
--
...
--
-- APPLY SEQUENCE ON LIVE:
--   1. supabase/m2-campaign-intents/showroom-allowlist-fix/03-verify-readonly.sql
--      (BEFORE — confirms current state / reads the allowlist)
--   2. THIS FILE (apply)
--   3. supabase/m2-campaign-intents/showroom-allowlist-fix/03-verify-readonly.sql
--      (AFTER — confirms 'showroom' is accepted; read-only)
-- ============================================================================
...
-- ----------------------------------------------------------------------------
-- 3) Defensive grants — idempotent, keep the anon/authenticated call path.
-- ----------------------------------------------------------------------------
...
-- ============================================================================
-- Done. Run 03-verify-readonly.sql AFTER this (read-only).
-- ============================================================================
